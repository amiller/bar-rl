...
if [[ -s "$BAR_DATA/interactions.jsonl" ]]; then
    OUT_NAME="$(basename "$REPLAY" .sdfz)-wasm"
    DEST="$PROJECT/traces/${OUT_NAME}.interactions.jsonl"
    mv "$BAR_DATA/interactions.jsonl" "$DEST"
    echo "interactions: $DEST ($(wc -l < "$DEST") lines)"
fi
